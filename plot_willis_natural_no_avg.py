...
## calculate decorrelation assuming natural weighting
  ...
  no_decor1 = numpy.zeros((len(dec), len(GMST)), numpy.float64);
  no_decor2 = numpy.zeros((len(dec), len(GMST)), numpy.float64);
  for idx in range(len(dec)):
    ...
    for i in range(len(LHA)):
      ...
      no_decor1[idx,i] = pi_t_const * numpy.mean(numpy.square(dudt * l1 + dvdt * m1  + dwdt * n1));
      no_decor2[idx,i] = pi_t_const * numpy.mean(numpy.square(dudt * l2 + dvdt * m2  + dwdt * n2));
#     no_decor1[idx,i] = pi_t_const * numpy.mean(numpy.square(dudt * l1 + dvdt * m1 ));
#     no_decor2[idx,i] = pi_t_const * numpy.mean(numpy.square(dudt * l2 + dvdt * m2 ));
...
# read in actual data
  ...
  for i in range(len(dec)):
    plt.plot(x, no_decor1[i,:], colours[i])
    plt.plot(x, no_decor2[i,:], colours[i])

# for i in range(len(dec)):
#   plt.plot(x, no_decor1[i,:])
#   plt.plot(x, no_decor2[i,:])
  plt.xlabel('Hour Angle (hours)')
  plt.ylabel('Maximum Residual (percent)')
  plt.title('Natural Wt Theoretical Analysis vs SKA Simulator with 0.14s Integration')
...
# plt.legend(loc=1)
  ...
  plot_file = 'natural_weighting_014sec_plot'

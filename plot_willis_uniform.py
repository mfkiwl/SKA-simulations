...
# read in actual data
  ...
  print('dec_values from BDA_utility_functions', dec_values)
  ...
  for i in range(len(dec_values)):
      ...
      if i < 5:
        try:
          print(x, y,plot_parm[i], ecs[i])
          plt.plot(x, y,plot_parm[i], label= ecs[i])
        except:
          continue
      else:
        try:
          print(x, y,plot_parm[i])
...
# plt.legend(bbox_to_anchor=(1.04,1), borderaxespad=0)
  if threshhold > 0:
    plot_file = 'sjw_willis_uniform_weighting_BDA_plot_' + str(threshhold)
  else:
    plot_file = 'Figure_4a_plot'
